...
{
#if 0
	static FProperty* HealthProperty = FindFieldChecked<FProperty>(UCharacterAttributeSet::StaticClass(), GET_MEMBER_NAME_CHECKED(UCharacterAttributeSet, Health));
	static FProperty* DamageProperty = FindFieldChecked<FProperty>(UCharacterAttributeSet::StaticClass(), GET_MEMBER_NAME_CHECKED(UCharacterAttributeSet, Damage));
	...
	if (DamageProperty == ModifiedProperty)
	{
		...
		if (Data.EvaluatedData.Magnitude > 0.f)
		{
			...
			const UCharacterAttributeSet* SourceAttributes = Data.EffectSpec.EffectContext.GetOriginalInstigatorAbilitySystemComponent()->GetSet<UCharacterAttributeSet>();
		...
		// At this point, the Magnitude of the applied damage may have been modified by us. We still do the translation to Health in UCharacterAttributeSet::PostAttributeModify.
	}
...
void UCharacterAttributeSet::GetLifetimeReplicatedProps(TArray< FLifetimeProperty >& OutLifetimeProps) const
{
	...
	DOREPLIFETIME( UCharacterAttributeSet, LifeSteal);
